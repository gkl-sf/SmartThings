...
//		fingerprint deviceId: "0x3101", inClusters: "0x70,0x32,0x60,0x85,0x56,0x72,0x86"
	...
	// Power row
        ...
        valueTile("energyOne", "device.energyOne", decoration: "flat") {
        	state(
        		"default", 
        		label: 'Since\n${currentValue}',     // def timeString = new Date().format("h:mm a", location.timeZone)    //	state.lastResetTime = "Since\n"+dateString+"\n"+timeString
			...
			if (cmd.sourceEndPoint == 1) {
				if (encapsulatedCommand.scale == 2 ) {
					...
					if (dispValue != state.energyL1Disp) {
						...
						if (state.display == 2) {
							[name: "energyOne", value: "z" + dispValue, unit: "", descriptionText: "zL1 Energy: ${formattedValue} kWh"]
						}
						else {
						}
					}
					...
					if (dispValue != state.energyL1Disp) {
						state.energyL1Disp = dispValue
						if (state.display == 2) {
							[name: "energyOne", value: "zz" + dispValue, unit: "", descriptionText: "zzL1 Energy: ${formattedValue} kVAh"]
						}
						else {
						}
					}
